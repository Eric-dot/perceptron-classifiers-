...
with open(Data, encoding='utf-8') as input_file:
    lines = input_file.readlines()
    count_dict = Counter()
    for line in lines:
        count_dict += get_word_count(line)
    features = dict(count_dict)
...
for k in range(0, 10):
    random.Random(k * 5).shuffle(lines)
    for line in lines:
        polarity['activation'] = 0
        origin['activation'] = 0
        polarity['y'] = 1 if line[13:16] == 'Pos' else -1
        origin['y'] = 1 if line[8:12] == 'True'  else -1
        word_count_dict = get_word_count(line)
        for word in word_count_dict:
